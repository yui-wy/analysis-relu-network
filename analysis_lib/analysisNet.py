...
class AnalysisNet(nn.Module):
    ...
    def _analysis_avgPool2d(self, x, module, pre_weight_graph=None, pre_bias_graph=None):
        ...
        ks = nn.modules.utils._pair(kernel_size)
        kernel_weight = torch.eye(in_channels, device=self._device).view(in_channels, out_channels, 1, 1) / (ks[0] * ks[1])
    ...
    def _analysis_BatchNorm2d(self, x, module, pre_weight_graph=None, pre_bias_graph=None):
        ...
        assert pre_weight_graph is not None, "BatchNorm2d is not a first layer!!"
        output = module(x)
        if pre_bias_graph is None:
            pre_bias_graph = torch.zeros_like(x, device=self._device)
        bias_graph = module(pre_bias_graph)
        ...
        eps = module.eps
        weight = module.weight if module.affine else torch.ones(num_features, device=self._device)
        # create weight_graph and bias_graph
        # (n, out_features, (*input_size))
        weight_graph = torch.zeros(graph_size, device=self._device)
    ...
    def _analysis_BatchNorm1d(self, x, module, pre_weight_graph=None, pre_bias_graph=None):
        """
        ...
        """
        assert module.track_running_stats, "Please set track_running_stats = True"
        assert pre_weight_graph is not None, "BatchNorm1d is not a first layer!!"
        output = module(x)
        if pre_bias_graph is None:
            pre_bias_graph = torch.zeros_like(x, device=self._device)
        bias_graph = module(pre_bias_graph)
    ...
    def _conv2d_opt(self, x, in_channels, out_channels, kernel_size, padding, stride, kernel_weight, graph_size, pre_weight_graph):
        ...
        if pre_weight_graph is None:
            pad_func = nn.ConstantPad2d(padding, 0)
            hook_x = pad_func(torch.zeros_like(x, device=self._device))
            hook_x = torch.zeros(hook_x.shape[0], out_channels, *hook_x.shape[1:], device=self._device)
            # origin
            for h in range(h_n):
                for w in range(w_n):
                    hook_x[:, :, :, h*stride[0]:h*stride[0]+kernel_size[0], w*stride[1]: w*stride[1]+kernel_size[1]] += kernel_weight
                    weight_graph[:, :, h, w] = hook_x[:, :, :, padding[0]:hook_x.shape[3]-padding[0], padding[1]:hook_x.shape[4]-padding[1]]
                    hook_x.zero_()
        # ===============================================================
        # speed 1
        # rm w loop
            ...
            for h in range(h_n):
                ...
                pre_graph_hook = pre_weight_graph[:, :, pos_h:pos_end, :, :, :, :].reshape(x.size(0), -1, self.size_prod).permute(0, 2, 1)
                # graph_hook : (n, c_out, w_out, *(input_size))
                weight_graph[:, :, h] = torch.matmul(pre_graph_hook, hook_kernel_weight_hook).reshape(x.size(0), *self._input_size, w_n, out_channels).permute(0, 5, 4, 1, 2, 3)
        return weight_graph
