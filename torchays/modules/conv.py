...
class AysConv2d(nn.Conv2d, base.AysBaseModule):
    ...
    def _conv2d_opt(self, x, input_size, output_size, weight_graph):
        ...
        if weight_graph is None:
            hook_x = torch.zeros(
                x.size(0), self.out_channels, self.in_channels, x.size(2) + self.padding[0] * 2, x.size(3) + self.padding[1] * 2, device=x.device
            )
            # origin
            for h in range(h_n):
                for w in range(w_n):
                    ...
                    wg[:, :, h, w] = hook_x[
                        :, :, :, self.padding[0] : hook_x.shape[3] - self.padding[0], self.padding[1] : hook_x.shape[4] - self.padding[1]
                    ]
                    hook_x.zero_()
        else:
            # hook_kernel_weight : (w_out, c_out, c_in, k , w_in+2padding)
            hook_kernel_weight = torch.zeros(
                w_n, self.out_channels, self.in_channels, self.kernel_size[0], x.size(3) + self.padding[1] * 2, device=x.device
            )
            for w in range(w_n):
            ...
            for h in range(h_n):
                ...
                wg[:, :, h] = (
                    torch.matmul(graph_hook, hook_kernel_weight_s).reshape(x.size(0), *input_size, w_n, self.out_channels).permute(0, 5, 4, 1, 2, 3)
                )
        return wg
