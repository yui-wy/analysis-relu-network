...
class AvgPool2d(nn.AvgPool2d, base.BaseModule):
    ...
    def _conv2d_opt(self, x, input_size, output_size, weight_graph):
        ...
        divisor = (
            ks[0] * ks[1] if self.divisor_override is None else self.divisor_override
        )
        kernel_weight = (
            torch.eye(channels, device=x.device).view(channels, channels, 1, 1)
            / divisor
        )
        # Conv2d opt
        # ===============================================================
        ...
        if weight_graph is None:
            hook_x = torch.zeros(
                ...
                channels,
                channels,
                x.size(2) + padding[0] * 2,
                x.size(3) + padding[1] * 2,
                device=x.device,
            )
            # origin
            for h in range(h_n):
                for w in range(w_n):
                    hook_x[
                        ...
                        :,
                        :,
                        h * stride[0] : h * stride[0] + ks[0],
                        w * stride[1] : w * stride[1] + ks[1],
                    ] += kernel_weight
                    wg[:, :, h, w] = hook_x[
                        :,
                        :,
                        :,
                        padding[0] : hook_x.shape[3] - padding[0],
                        padding[1] : hook_x.shape[4] - padding[1],
                    ]
                    hook_x.zero_()
        # ===============================================================
        else:
            # hook_kernel_weight : (w_out, c_out, c_in, k , w_in+2padding)
            hook_kernel_weight = torch.zeros(
                w_n,
                channels,
                channels,
                ks[0],
            ...
            for h in range(h_n):
                ...
                wg[:, :, h] = (
                    ...
                    .reshape(x.size(0), *input_size, w_n, channels)
                    .permute(0, 5, 4, 1, 2, 3)
                )
        return wg
